...
export function StudyProvider({ children }: { children: ReactNode }) {
  ...
  const addSubject = useCallback((s: Omit<Subject, 'id' | 'completedChapters'>) => {
    upd((prev) => ({
      ...prev,
      subjects: [...prev.subjects, { ...s, id: crypto.randomUUID(), completedChapters: 0 }],
    }))
  ...
  const addChapter = useCallback((c: { name: string; subjectId: string; difficulty: Difficulty; notes: string }) => {
    upd((prev) => ({
      ...
      chapters: [
        ...prev.chapters,
        { ...c, id: crypto.randomUUID(), status: 'not-started' as ChapterStatus, revisionCount: 0 },
      ],
    }))
  ...
  const deleteChapter = useCallback((id: string) => {
    upd((prev) => {
      ...
      return {
        ...
        subjects: wasCompleted && ch
          ? prev.subjects.map((s) =>
              s.id === ch.subjectId
                ? { ...s, completedChapters: Math.max(0, s.completedChapters - 1) }
                : s
            )
          : prev.subjects,
      }
    ...
    upd((prev) => {
      ...
      return {
        ...
        chapters: prev.chapters.map((c) =>
          c.id === id ? { ...c, status: 'completed', completedAt, revisionCount: c.revisionCount + 1 } : c
        ),
        subjects: prev.subjects.map((s) =>
          s.id === chapter.subjectId ? { ...s, completedChapters: s.completedChapters + 1 } : s
        ),
  ...
  const addTask = useCallback((t: { title: string; estimatedMinutes: number }) => {
    upd((prev) => ({
      ...prev,
      tasks: [...prev.tasks, { ...t, id: crypto.randomUUID(), completed: false, date: todayStr() }],
    }))
  ...
  const addGoal = useCallback((g: Omit<Goal, 'id' | 'progress'>) => {
    ...
    upd((prev) => ({
      ...prev,
      goals: [...prev.goals, { ...g, id: crypto.randomUUID(), progress }],
    }))
  ...
  const completeRevision = useCallback((id: string) => {
    upd((prev) => ({
      ...prev,
      revisions: prev.revisions.map((r) => r.id === id ? { ...r, completed: true } : r),
    }))
  ...
  const completeFocusSession = useCallback((session: { date: string; duration: number; mode: FocusMode }) => {
    ...
    if (session.mode === 'focus') {
      addToast(`🎉 Focus session complete! +${session.duration} min logged.`, 'success')
    }
  }, [addToast])
